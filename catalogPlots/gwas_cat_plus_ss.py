...
def fetch_sumstats_status():
    ...
    study_sumstats_dict = {}
    api_params = {"fullPvalueSet": "true", "size": 1000}
    ...
    logger.info("Page {} of {}".format(api_response["page"]["number"], api_response["page"]["totalPages"]))
    study_sumstats_dict.update(parse_api_response(api_response))
    while api_response["_links"]["next"]:
        api_response = requests.get(api_response["_links"]["next"]["href"]).json()
        logger.info("Page {} of {}".format(api_response["page"]["number"], api_response["page"]["totalPages"]))    
        study_sumstats_dict.update(parse_api_response(api_response))
    sumstats_status_df = pd.DataFrame(study_sumstats_dict.items(), columns=['study_accession', 'sumstats_bool'])
...
def main():
    ...
    merge_sumstats_status_with_study_table()
