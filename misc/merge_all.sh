#!/bin/bash
#SBATCH --job-name=merge_gwas_summary_stats
...
GWAS_DIR="${BASE_DIR}gwas_summary_stats/"
COMBINED_DIR="${BASE_DIR}combined/gwas_summary_stats/"

# Ensure the combined directory exists
mkdir -p "$COMBINED_DIR"

# Updated ancestries list to include "EAS"
...
for GWAS_NAME in $(ls $GWAS_DIR); do
    ...
    for ANCESTRY in "${ANCESTRIES[@]}"; do
        ...
        FILES=$(ls ${DIR}*_${ANCESTRY}.txt.gz 2> /dev/null)
        ...
        TEMP_FILE="${DIR}temp_${ANCESTRY}.txt"
        ...
        if [ "$(wc -l <"$TEMP_FILE")" -gt 1 ]; then
            gzip -c "$TEMP_FILE" > "${COMBINED_DIR}${GWAS_NAME}_${ANCESTRY}_combined.txt.gz"
        else
